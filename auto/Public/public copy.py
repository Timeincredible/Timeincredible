...
LastEditors: wwt
LastEditTime: 2021-07-20 18:46:59
'''
import os
import re
import json
...
class BaseHTML():
    ...
    def delTag(self,tagName,**attrs):
        ...
        if id_:
            ...
            tdTag = self.soup.find(tagName,**attrs)
        if tdTag:
            tdTag.decompose()
    ...
    def replaceText(self,name,replaceName):
        ...
        self.soup = BeautifulSoup(texts,"html.parser")
    ...
    def editProinfo(self):
        ...
        # 5. 删除订单编号
        res = self.soup.find(string="订单编号 ")
        if res:
            parentTag =  res.parent.parent.parent
            parentTag.decompose()

        # 1 设置 项目名称 和 子项目编号
        projectTag = self.soup.find(string="项目名称 ")
        if projectTag:
            parentTag = projectTag.parent.parent.parent
            parentTag.string=""
        ...
        parentTag.append(BeautifulSoup(replaceProjectTem,"html.parser")) 

        # 2 设置 客户姓名 和 单位
        projectTag = self.soup.find(string="客户姓名 ")
        if projectTag:
            parentTag = projectTag.parent.parent.parent
            parentTag.string=""
        ...
        parentTag.append(BeautifulSoup(replaceCustomerTem,"html.parser")) 

        # 3 设置 收样人 和 收样日期 (收样日期根据我们的收样单进行修改)
        ReceivedTem = '''<td><p align="center">收样人 </p></td>\n<td><p align="center">.*</p></td>\n<td><p align="center">收样日期 </p></td>\n<td><p align="center">.*</p></td>'''
        projectTag = self.soup.find(string="收样人 ")
        if projectTag:
            parentTag = projectTag.parent.parent.parent
            parentTag.string=""
            ...
            parentTag.append(BeautifulSoup(replaceReceivedTem,"html.parser")) 

        # 4 设置 审核日期 改成 与 报告日期同一天
        ...
        soup = re.sub(auditTem,replaceAuditTem,str(self.soup))
...
class TranscriptomeQuality(BaseHTML):
    ...
    def main(self,anapath,parentDir):
        try:
            ...
            #=================== 二、网页修改 ===================
            # 1.报告时间修改。
            pTag=  self.soup.find("p",**{"align":"right"})
            bTag = pTag.find("b")
            if bTag:
                bTag.decompose()
            bTag = BeautifulSoup('<b>报告时间：%s</b>'%(self.yesterda),"html.parser") 
            pTag.append(bTag) 
